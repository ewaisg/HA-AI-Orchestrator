...
| ENV-003 | `PARTIAL` | LM-LIVE-001 through LM-LIVE-008 and LM-PROBE-001 through LM-PROBE-002 in `docs/evidence/2026-08-23-lm-studio-environment.md` | Current server/model and bounded schema/tool-call capabilities are verified. Authentication is disabled on an all-interface plain-HTTP listener and the host application allow rules are over-broad; remediate both and revalidate from Home Assistant before LOC-003 can complete |
