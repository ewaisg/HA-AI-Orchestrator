...
| ENV-010 | `PARTIAL` | HA-LIVE-006, HA-LIVE-022 through HA-LIVE-024, HA-LIVE-028 through HA-LIVE-029, and HA-OWNER-001 through HA-OWNER-003 | Backup encryption/emergency-kit/restore readiness and any specific LAN firewall/VLAN rules |
| ENV-003 | `PARTIAL` | LM-LIVE-001 through LM-LIVE-007 and LM-PROBE-001 through LM-PROBE-002 in `docs/evidence/2026-08-23-lm-studio-environment.md` | Current server/model and bounded schema/tool-call capabilities are verified. Authentication is disabled on an all-interface plain-HTTP listener; enable authentication and revalidate from Home Assistant before LOC-003 can complete |
