...
| ENV-003 | `PARTIAL` | LM-LIVE-001 through LM-LIVE-015 and LM-PROBE-001 through LM-PROBE-002 in `docs/evidence/2026-08-23-lm-studio-environment.md` | Authentication rejects missing and invalid credentials, the restricted token denies both provider-side MCP classes, and the restored secret-backed Home Assistant path returned HTTP `200`. Narrow and reinspect the host firewall before LOC-003 can complete |
