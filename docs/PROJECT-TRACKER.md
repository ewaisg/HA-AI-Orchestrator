...
Current resume point: **LOC-001 third candidate `62df5da1dd1e1b337b81bf65bd53fae4a888fadb` was rejected at `2026-08-24T17:19:51Z` for `__class__` type spoofing and at `2026-08-24T17:20:19Z` for missing distinct unhashable-code regression coverage. The working tree now uses exact `type(code) is ErrorCode` and runs setup, initial setup, reauth, and reconfigure against genuinely unhashable, throwing-hash, and type-spoofing throwing-hash codes; 62 focused clean-Linux tests pass. Create a new immutable artifact, reproduce all gates, and repeat both reviews. No LM Studio transport, endpoint, credential, model, provider UI, or Home Assistant action is present.**
...
|---|---|---|---|---|
| LOC-001 | Complete local provider connection lifecycle on the validated skeleton | `REVIEW` | FND-010, LOC-002 | Candidate `62df5da1dd1e1b337b81bf65bd53fae4a888fadb` passed 242 full/54 focused but workflow/safety found `isinstance` type spoofing and test/release found missing distinct unhashable coverage. The current tree uses exact type identity and exercises three hostile code classes across all four lifecycle paths; 62 focused clean-Linux, 167 local provider/security/quality, 167 pure, Ruff, canary, and diff pass. Next: new immutable artifact, full gates, fresh reviews. Evidence: `docs/evidence/2026-08-24-loc-001-provider-entry-lifecycle.md`. |
