...
Current resume point: **Clarify the exact FND-011 mobile client and whether `None contacted` appeared there. All other same-version panel lifecycle checks are confirmed on Home Assistant OS / Core 2026.8.3 / Supervisor 2026.07.5 / OS 18.2 / Frontend 20260729.7. Leave upgrade survival pending until an actual Core update is available, and claim FND-012 as the next independently actionable Phase 0 task. Do not begin LOC-001 until FND-010 passes.**
...
| Evidence/input needed | Exact FND-011 mobile client and mobile `None contacted` result; later Core-upgrade evidence; exact window/Echo entity and action identifiers; later LM Studio non-secret settings; and, before FND-010, ENV-009 privacy/retention choices plus remaining ENV-010 backup/network evidence |
...
| FND-011 | Prove bundled panel registration and compatibility boundary | HA + UI | `IN PROGRESS` | FND-015, ENV-001 | Exact live matrix is recorded in `docs/evidence/2026-08-23-fnd-011-panel-lifecycle.md`. All same-version scenarios except two mobile details are confirmed without a matching log issue. Clarify the client and mobile summary result; upgrade survival remains pending until a real Core update is available. |
