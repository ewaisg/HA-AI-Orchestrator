...
Current resume point: **FND-007 is complete: ENV-003, ENV-009, and Phase 0 ENV-010 are resolved with redacted live evidence. The authenticated Home Assistant-to-LM Studio path has bounded positive/negative behavior, restricted provider permissions, a source-scoped Private-only TCP firewall rule, successful post-firewall HTTP `200`, and same-private-`/24` interfaces requiring no inter-subnet router hop. Home Assistant's Recommended daily encrypted backups retain three copies; the first `87.89 MB` automatic backup completed to two locations, emergency-kit custody outside Home Assistant is owner-confirmed, and isolated restore testing is required every six months and after major changes. The first restore artifact is due by 2027-02-23 unless triggered earlier. Next: claim FND-013 for the redacted data-flow and requirement/control/test traceability records. FND-011 remains open only for an actual Core upgrade artifact, so FND-010 cannot close yet. Do not begin LOC-001 until FND-010 passes.**
...
| Active work | None; next claim FND-013 redacted data-flow and control-to-test traceability |
