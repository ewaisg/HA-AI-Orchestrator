...
Current resume point: **FND-007 now has resolved ENV-009 policy defaults, the owner-confirmed Home Assistant-to-LM Studio LAN topology, and partial ENV-003/ENV-010 live evidence. The exact Home Assistant LAN source is privately known. Backup setup remains incomplete with zero automatic backups. The loaded model passed bounded JSON-schema and no-execution tool-call probes. LM Studio authentication rejects a missing token, the restricted token denies both provider-side MCP permission classes, and both secret-backed Home Assistant REST commands loaded without a reported error. A synthetic Home Assistant-origin request returned HTTP `200`; related log searches were clear and the foundation panel remained healthy. Next: add an isolated Home Assistant-origin missing/invalid-token failure test without exposing or replacing the valid secret, then narrow and reinspect the broad Windows Firewall rules. Backup encryption, emergency-kit, automatic-backup, restore-test, and router/VLAN facts remain open. Do not claim dependent FND-013 until FND-007 is complete. FND-011 remains open only for an actual Core upgrade artifact. Do not begin LOC-001 until FND-010 passes.**
...
| Evidence/input needed | Later FND-011 Core-upgrade evidence; an isolated negative Home Assistant-origin authentication test and a narrow inspected Windows Firewall rule before LOC-003; plus backup encryption, emergency-kit, automatic-backup, restore-test, and router/VLAN facts before FND-010 |
...
| FND-007 | Gather Phase 0 live environment facts | User + HA specialist | `IN PROGRESS` | — | ENV-001, ENV-004 discovery/action contract, Phase 0 ENV-007, and ENV-009 are resolved. ENV-003/ENV-010 are partial in `docs/evidence/2026-08-23-lm-studio-environment.md`: schema/no-execution tool-call probes passed; authentication rejects a missing token; the restricted token denies both provider-side MCP classes; and the secret-backed Home Assistant path returned HTTP `200` with clear scoped logs and a healthy panel. Home Assistant's exact LAN source was privately verified. Backup setup remains incomplete with zero automatic backups. Exact private identifiers were withheld. Next: add an isolated HA-origin negative authentication test and narrow/reinspect the firewall; backup encryption, emergency-kit, automatic-backup, restore-test, and router/VLAN facts remain open |
