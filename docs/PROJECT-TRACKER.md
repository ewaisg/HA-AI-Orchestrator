...
Current resume point: **LOC-003 immutable synthetic artifact `598a3259de25e9f1060a4b670a2b13dace4808ca` passes exact clean Linux: 315 full, 72 focused lifecycle, 192 provider, 54 adapter-focused, 30 security/evidence/traceability, and 222 pure tests plus Ruff, canary, hashes, schema, traceability, and diff. Obtain independent workflow/safety and test/release pre-live approvals, then push a testable bundle and obtain redacted live Home Assistant revalidation. No live adapter request has been made by this implementation.**
...
| LOC-003 | Add authenticated LM Studio/OpenAI-compatible adapter | `IN PROGRESS` | LOC-001, LOC-002, ENV-003 | Artifact `598a3259de25e9f1060a4b670a2b13dace4808ca` uses the shared HA session; private-address-only SSRF boundary; exact `/v1` paths; required backend-only token/model config; no redirects; 60-second and 2 MiB bounds; strict JSON; safe normalized failures; configured-model validation; non-streaming generation; structured output; and tool-request parsing without execution. Exact clean Linux passes 315 full, 72 focused lifecycle, 192 provider, 54 adapter-focused, 30 security/evidence/traceability, and 222 pure tests. Next: independent pre-live reviews, push/install, and redacted live revalidation. Evidence: `docs/evidence/2026-08-24-loc-003-lm-studio-adapter.md`. |
