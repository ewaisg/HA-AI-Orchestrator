...
Current resume point: **FND-007 now has resolved ENV-009 policy defaults, the owner-confirmed Home Assistant-to-LM Studio LAN topology, and partial ENV-003/ENV-010 live evidence. The exact Home Assistant LAN source is privately known. Backup setup remains incomplete with zero automatic backups. The loaded model passed bounded JSON-schema and no-execution tool-call probes. The owner enabled LM Studio authentication, created a token that denies both provider-side MCP permission classes, and saved the complete Bearer value under a Home Assistant secret key; a missing-token request now returns `401`. Next: open `configuration.yaml`, add the secret-backed Authorization header to both LM Studio `rest_command` definitions, validate/load the YAML, run authenticated and negative Home Assistant-origin tests, then narrow and reinspect the broad Windows Firewall rules. Do not claim dependent FND-013 until FND-007 is complete. FND-011 remains open only for an actual Core upgrade artifact. Do not begin LOC-001 until FND-010 passes.**
...
| Evidence/input needed | Later FND-011 Core-upgrade evidence; secret-backed Home Assistant headers, authenticated/negative Home Assistant-origin revalidation, and a narrow inspected Windows Firewall rule before LOC-003; plus an owner backup policy and router/VLAN facts before FND-010 |
...
| FND-007 | Gather Phase 0 live environment facts | User + HA specialist | `IN PROGRESS` | — | ENV-001, ENV-004 discovery/action contract, Phase 0 ENV-007, and ENV-009 are resolved. ENV-003/ENV-010 are partial in `docs/evidence/2026-08-23-lm-studio-environment.md`: schema/no-execution tool-call probes passed; authentication now rejects a missing token; the created token denies both provider-side MCP classes; and Home Assistant holds the Bearer value under a secret key. Home Assistant's exact LAN source was privately verified. Backup setup remains incomplete with zero automatic backups. Exact private identifiers were withheld. Next: add both secret-backed headers, run HA-origin tests, and narrow/reinspect the firewall; owner backup policy and router facts remain open |
