...
Current resume point: **FND-007 now has resolved ENV-009 policy defaults, the owner-confirmed Home Assistant-to-LM Studio LAN topology, and partial ENV-003/ENV-010 live evidence. The exact Home Assistant LAN source is privately known. Backup setup remains incomplete with zero automatic backups. The loaded model passed bounded JSON-schema and no-execution tool-call probes, but authentication is disabled, per-request MCPs are enabled contrary to DEC-012, and the Windows Firewall application rules allow LM Studio inbound TCP/UDP on any port from any remote IP on Private and Public profiles. Owner handoff: disable per-request MCPs, enable authentication, create a token without sharing it in chat, and place the complete Bearer value in Home Assistant `secrets.yaml`; then update the two current `rest_command` headers, validate configuration, narrow the firewall, and re-run authorized/unauthorized Home Assistant-origin tests. Do not claim dependent FND-013 until FND-007 is complete. FND-011 remains open only for an actual Core upgrade artifact. Do not begin LOC-001 until FND-010 passes.**
...
| Evidence/input needed | Later FND-011 Core-upgrade evidence; owner completion of the LM Studio authentication/per-request-MCP handoff, secret-backed Home Assistant headers, a narrow Windows Firewall rule, and authenticated Home Assistant-origin revalidation before LOC-003; plus an owner backup policy and router/VLAN facts before FND-010 |
...
| FND-007 | Gather Phase 0 live environment facts | User + HA specialist | `IN PROGRESS` | — | ENV-001, ENV-004 discovery/action contract, Phase 0 ENV-007, and ENV-009 are resolved. ENV-003/ENV-010 are partial in `docs/evidence/2026-08-23-lm-studio-environment.md`: live schema and no-execution tool-call probes passed, while API authentication is disabled, per-request MCPs conflict with DEC-012, and broad Windows Firewall application rules require remediation. Home Assistant's exact LAN source was privately verified. Backup setup remains incomplete with zero automatic backups. Exact private identifiers were withheld. Next: owner completes the LM Studio security handoff, then authenticated Home Assistant/firewall revalidation; owner backup policy and router facts remain open |
