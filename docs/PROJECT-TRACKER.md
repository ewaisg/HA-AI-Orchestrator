...
Current resume point: **FND-007 now has resolved ENV-009 policy defaults, the owner-confirmed Home Assistant-to-LM Studio LAN topology, and partial ENV-003/ENV-010 live LM Studio evidence. The loaded model passed bounded JSON-schema and no-execution tool-call probes, but authentication is disabled and the enabled Windows Firewall application rules allow LM Studio inbound TCP/UDP on any port from any remote IP on Private and Public profiles. Next, coordinate API-token enablement, narrow the firewall to the actual Home Assistant-to-TCP-1234 private-LAN path, then revalidate from Home Assistant. Also finish backup encryption/emergency-kit/restore readiness and router/VLAN facts. Do not claim dependent FND-013 until FND-007 is complete. FND-011 remains open only for an actual Core upgrade artifact. Do not begin LOC-001 until FND-010 passes.**
...
| Evidence/input needed | Later FND-011 Core-upgrade evidence; LM Studio API-token enablement, narrow Windows Firewall rule, and authenticated Home Assistant-origin revalidation before LOC-003; and, before FND-010, remaining ENV-010 backup encryption/emergency-kit/restore plus router/VLAN facts |
...
| FND-007 | Gather Phase 0 live environment facts | User + HA specialist | `IN PROGRESS` | — | ENV-001, ENV-004 discovery/action contract, Phase 0 ENV-007, and ENV-009 are resolved. ENV-003/ENV-010 are partial in `docs/evidence/2026-08-23-lm-studio-environment.md`: live schema and no-execution tool-call probes passed, while API authentication is disabled and broad Windows Firewall application rules require remediation. ENV-002 plus backup/router facts remain partial in `docs/evidence/2026-08-22-home-assistant-environment.md`. Exact private identifiers were withheld. Next: coordinate LM Studio authentication/firewall changes and finish backup/router evidence |
