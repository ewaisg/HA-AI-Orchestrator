...
| Field | Current value |
|---|---|
| Last completed | 2026-09-05 second session: live config-entry reload lifecycle for LOC-003/LOC-004 (runtime-reset semantics, post-reload retest, clean logs), and live catalogue search/refresh/rendering acceptance for LOC-005 (2,193/173/19 counts, domain-prefix search, stable refresh, no viewport overflow). Evidence appended to each task's existing evidence file. |
| Active work | LOC-007 Phase 1 release gate, gated on LOC-003/004/005 and COMP-001 remaining named live acceptance items. |
| Evidence/input needed | LOC-003/004 still need a full Home Assistant Core **restart** (not just config-entry reload), bounded timeout/cancellation reproduction, duplicate-click protection, and Companion App Android rendering. LOC-005 still needs a real registry mutation check (rename/removal/disable/unavailable/area-device change) and Companion App Android rendering. COMP-001 still needs the same live restart/panel/lifecycle/Android acceptance on Core 2026.9.0. First isolated backup restore artifact remains due by 2027-02-23 or earlier after a major change. |
| Next gate | Perform a full Core restart and observe all three panels recover cleanly; perform one reversible registry rename/disable and observe the catalogue reflect it; obtain Companion App Android access; then run the LOC-007 quality/redaction/smoke gate. |
...
| LOC-003 | Add authenticated LM Studio/OpenAI-compatible adapter | `REVIEW` | LOC-001, LOC-002, ENV-003 | Exact candidate `989917fd4229f528c142a9ecebeeea3934c394da` validates historical tool arguments before serialization, rejects noncanonical JSON with zero network requests, and has both independent pre-live approvals. Shared exact-source gates: 340 full, 98 focused, 30 security/evidence/traceability, 231 pure, Ruff/canary. 2026-09-05 live config-entry reload verified correct post-reload `not_tested` state, a subsequent passing connection test, and clean scoped logs. Full Core restart, bounded timeout/cancellation reproduction, and Companion App Android rendering remain. Evidence: `docs/evidence/2026-08-24-loc-003-lm-studio-adapter.md`; manifest `LOC-003-LM-STUDIO-ADAPTER-002.json`. |
| LOC-004 | Add provider setup/test UI | `REVIEW` | LOC-001, LOC-002 | Exact candidate `989917fd4229f528c142a9ecebeeea3934c394da` reports `not_tested` until an explicit timestamped observation, preserves evidenced health on Home Assistant transport failure, fails closed on malformed adapter outcomes, and prevents late in-flight tests from crossing unload/reload ownership. Both independent pre-live reviewers approved it. Frontend passes 95 tests and byte identity. 2026-09-05 live testing confirmed runtime-reset semantics (no stale `healthy` after reload), a passing re-test, correct desktop (1400x900) and narrow-viewport (390x844, no horizontal overflow) rendering, and clean scoped logs. Duplicate-click protection, transport-failure reproduction, Companion App Android, and full Core restart remain. Evidence: `docs/evidence/2026-08-25-loc-004-provider-ui.md`; manifest `LOC-004-PROVIDER-UI-002.json`. |
| LOC-005 | Add read-only entity/area/device catalog | `REVIEW` | LOC-001 | Exact candidate `989917fd4229f528c142a9ecebeeea3934c394da` provides stable registry identity, effective area/device relationships, availability, integration/domain metadata, search, responsive layout, explicit `AI access: none`, and strict privacy omissions. Both independent pre-live reviewers found no action, state-value, secret, or authorization regression. 2026-09-05 live testing recorded a redacted inventory of 2,193 entities / 173 devices / 19 areas, live domain-prefix search filtering, a stable-count refresh, and clean desktop/narrow-viewport rendering with no console errors. Rename/removal/disable/unavailable/area-device-change checks (which require a real registry mutation) and Companion App Android rendering remain. Evidence: `docs/evidence/2026-08-27-loc-005-registry-catalog.md`, `docs/evidence/2026-08-28-loc-005-read-only-catalog.md`; manifest `LOC-005-REGISTRY-CATALOG-002.json`. |
