...
Current resume point: **WFL-002C live acceptance is closed (owner-reported 2026-09-12). Next: the remaining Phase 1 live items on the owner's instance, in this order: LOC-008 Android confirmation (only the transcript scrolls, composer usable with the keyboard open, spacing consistent); LOC-004 duplicate-click and transport-failure observation; LOC-003 bounded timeout/cancellation reproduction; LOC-005 removal/disable/unavailable/area-device-change checks. Then LOC-007 (quality gate, redaction check, Home Assistant Green smoke test, install instructions). LOC-007 still gates any workflow runtime activation.**
...
| Field | Current value |
|---|---|
| Last completed | WFL-002 code: persisted workflow store, foundation-owned observation-only activation, admin API, and panel list/editor integration with 622 backend and 184 browser tests. Before that, WFL-002C: owner installed the September 11 archive and reported evening passes, daytime blocks, editors work on desktop and Android (ENV-014 resolved). Before that, LOC-004: the September 9 providers-view repair (same-tick duplicate-test guard, confirmed health retained across a later transport failure) now has two browser regression tests proven by mutation; full gates rerun on the committed tree in a Linux container: 576 backend (Core 2026.8.3 only), 388 pure, 152 frontend, byte-identical bundle, Ruff/canary. Before that: WFL-002B harness verified and WFL-002C editor/API implemented and reviewed. |
| Active work | WFL-002 review and ENV-015 live acceptance; remaining Phase 1 live acceptance (LOC-008, LOC-004, LOC-003, LOC-005); then LOC-007. |
...
| Next gate | Close remaining LOC-003/004/005/008 live acceptance, then LOC-007 before any workflow runtime activation. |
| Production code | Foundation, provider lifecycle/contract, authenticated LM Studio, provider setup/test UI, read-only catalogue, bounded admin local text chat, offline workflow previews, and stored workflows with observation-only activation are implemented. Workflow step execution (AI steps, notifications), device actions, Assist, cloud providers and chat persistence remain planned. |
| Repository | The observer harness, offline preview API/UI/tests, bundle and docs are committed as `1f71fc4`. The September 11 LOC-004 regression tests and this tracker update follow on `claude/sharp-sagan-xl2abm`. The install ZIPs referenced by evidence live under the gitignored `outputs/` directory on the owner's workstation and are not in the repository. No live installation performed. |
...
| LOC-004 | Add provider setup/test UI | `REVIEW` | LOC-001, LOC-002 | September 9 commit `d73890b` repaired loss of last-confirmed health after a subsequent transport failure and same-tick duplicate test submissions, but shipped no tests and no recorded review. September 11 added two browser regression tests that fail with the repair reversed and pass with it; full gates pass on the committed tree (see the 2026-09-11 addendum in the LOC-004 evidence). A separate reviewer agent then approved the repair with non-blocking follow-ups (recorded in the same addendum); the test overclaim it found was fixed and re-proven by mutation. Historical exact candidate `989917fd4229f528c142a9ecebeeea3934c394da` reports `not_tested` until an explicit timestamped observation, preserves evidenced health on Home Assistant transport failure, fails closed on malformed adapter outcomes, and prevents late in-flight tests from crossing unload/reload ownership. Both independent pre-live reviewers approved it. Frontend passes 95 tests and byte identity. 2026-09-05 live testing confirmed runtime-reset semantics across both a config-entry reload and a full Core restart (no stale `healthy` after either), passing re-tests, correct desktop (1400x900) and narrow-viewport (390x844, no horizontal overflow) rendering, and clean scoped logs; the owner separately reported Companion App Android rendering as fine. Live duplicate-click and transport-failure reproduction on the owner's instance remain. Evidence: `docs/evidence/2026-08-25-loc-004-provider-ui.md`; manifest `LOC-004-PROVIDER-UI-002.json`. |
...
| LOC-008 | Fix chat layout: app-shell scrolling and Home Assistant-consistent density | `REVIEW` | LOC-006 | Owner reported that on mobile the whole chat page scrolled instead of only the message area, and that desktop spacing was too loose versus the rest of the app. The chat view is now an app shell: the panel host is sized to the space Home Assistant actually gives it, the section header/provider controls/composer stay fixed, and only the transcript scrolls. Density was tightened, the composer auto-grows and resets, new turns auto-scroll into view, and the privacy text collapses into a disclosure whose destination line stays visible. Frontend passes 116 browser tests (4 new layout regressions), lint, typecheck, build, sync and byte identity; bundle is 95,427 bytes, SHA-256 `d24d6b50aa09cf805822f7ec06af94bfdd84813c153e700d328003701677dce2`. September 5 evidence records live install and desktop verification; owner confirmation on Companion App Android remains. Evidence: `docs/evidence/2026-09-05-loc-008-chat-layout.md`. |
| LOC-009 | Make panel bundle updates cache-safe | `DONE` | LOC-008 | Owner: primary + backend subagent; claimed 2026-09-09. Committed hash implementation repaired to read through the HA executor; real changed-byte tests and independent review pass. Full backend: 383 tests on each Core 2026.8.3 and 2026.9.0; frontend: 116; pure: 234. Owner explicitly confirmed updated panel/chat worked without clearing caches after candidate handoff. Evidence: `docs/evidence/2026-09-09-loc-009-resume.md`. Home Assistant's Workbox service worker (`/sw-modern.js`, `workbox-runtime` cache) served a stale panel bundle after a verified on-disk replacement. A config-entry reload, a full Core restart, `cache: 'no-store'`, and a CDP browser-cache clear all failed to dislodge it; only deleting the cached entry worked. `cache_headers=False` does not defeat the service worker. Add a content hash or version query string to `PANEL_MODULE_URL` so each build requests a distinct URL, plus a documented cache-clear step for users. Required evidence: an automated test that the module URL changes when the bundle changes, and a live update observed without any manual cache clearing. Context: `docs/evidence/2026-09-05-loc-008-chat-layout.md`. |
| LOC-007 | Phase 1 release gate | `TODO` | LOC-003 through LOC-006, LOC-008, LOC-009, DOC-001 | Quality gate, redaction check, Home Assistant Green smoke test, and user-facing install instructions that include the service-worker cache-clear step |
...
| WFL-002 | Implement curated deterministic triggers and conditions | `IN PROGRESS` | WFL-001 | Owner directed on 2026-09-12 to continue toward running features while the LOC-007 live items stay open. Backend landed 2026-09-12: `workflow_store.py` (HA `Store`, atomic writes, fail-closed load, 50-document bound), `workflow_manager.py` (foundation-owned activation of enabled workflows using the WFL-002B observer, cleanup retry, unreadable-store isolation), `workflow_api.py` (admin-only list/save/delete/set_enabled/run_manual). 46 new HA tests cover persistence, corruption, activation, replace/disable/delete, activation and cleanup failure, foundation unload and simulated restart. Full backend 622 on Core 2026.8.3. No step executes: no provider call, notification or HA action; `features.workflows` stays false until WFL-003/WFL-004 and LOC-007. Panel landed the same day: stored workflows list with enable/disable, observe now, load into editor, two-step delete; editor `Save as workflow`; 184 browser tests; bundle 122,271 bytes SHA-256 `5eee8a4a...d0cae`. Evidence: `docs/evidence/2026-09-12-wfl-002-stored-workflows.md`. Remaining: independent review record and ENV-015 live save/enable/observe/restart on the owner's instance. |
...
| WFL-002C | Expose offline workflow preview in Automations | `DONE` | WFL-002A | Admin-only bounded WebSocket API and editor with synthetic examples implemented; 20 API/34 UI focused tests, 150 full frontend, 576 full backend on each Core pass; independent review approved. 2026-09-12 owner installed archive `ai-orchestrator-7150189.zip` and reported evening passes, daytime blocks, editors work on desktop and Android (owner-reported, ENV-014 resolved). Lifecycle-probe accessibility after install was not reported. Evidence: `docs/evidence/2026-09-09-wfl-002bc-panel-observer.md`. |
