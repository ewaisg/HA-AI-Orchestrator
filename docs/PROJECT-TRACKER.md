...
Current resume point: **FND-013 remains in REVIEW. Committed artifact revision `6ba4793a3d5b283c91fc5a78dc141c80644f2c20` contains the corrected redacted map/catalog/schema/tests and canonical LF evidence hashes. It resolves all findings from superseded reviews: backup/restore direction and trust zones; transient secret versus stored/transport handling; same-version versus actual Core-upgrade proof; prompt injection; storm/concurrency on event, provider, action, and notification boundaries; supply chain; device effects; correct upgrade residual; and cross-platform artifact hashing. Checks: 87 pure tests, six focused integrity tests, Ruff, canary, diff, privacy, and working/index hash equality passed. The incomplete manifest points to that clean revision with `dirty: false`; both independent reviews are pending. FND-011 remains open only for an actual Core upgrade artifact, so FND-010 cannot close yet. Do not begin LOC-001 until FND-010 passes.**
...
| FND-013 | Define data-flow and control-to-test traceability records | Security + tracker | `REVIEW` | FND-007 | Clean artifact revision `6ba4793a3d5b283c91fc5a78dc141c80644f2c20`: 7 data classes, 13 nodes, 18 flows, 19 requirements, 19 controls, 23 tests; all prior safety and reproducibility findings corrected. 87 pure tests and six focused integrity tests passed; Ruff/canary/diff/privacy/hash checks passed. Manifest is committed-source, clean, and incomplete pending both independent approvals |
