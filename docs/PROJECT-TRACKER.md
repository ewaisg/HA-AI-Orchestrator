...
Current resume point: **LOC-005 artifact `238928b518ee9b9c3e64e5a4dd3d52b9fcae0288` is implemented and its synthetic gates pass; it is awaiting independent workflow/safety and test/release review plus redacted live registry, rename/removal, disable, and unavailable evidence. LOC-003/LOC-004 live positive setup, invalid-key rejection, valid-key reconfiguration, provider testing, and browser-reload persistence passed; their remaining lifecycle, mobile, screenshot, and log checks remain open.**
...
| Active work | LOC-005 exact artifact in independent review; LOC-003/LOC-004 live acceptance remains partially passed |
...
| Next gate | Independently review LOC-005 artifact `238928b518ee9b9c3e64e5a4dd3d52b9fcae0288`, then install and record live registry/rename evidence; separately complete remaining LOC-003/LOC-004 live acceptance |
...
| LOC-003 | Add authenticated LM Studio/OpenAI-compatible adapter | `REVIEW` | LOC-001, LOC-002, ENV-003 | Remediated artifact `53fe6781930f9264daf96ddd9f79f3aa7f691c33` uses the shared HA session; private-address-only SSRF boundary; exact `/v1` paths; required backend-only token/model config; no redirects; 60-second and 2 MiB bounds; strict bounded JSON; safe normalized failures; configured-model validation; non-streaming generation; structured output; and tool-request parsing without execution. The first pre-live review found nonfinite-number and excessive-nesting gaps; the remediated artifact rejects both provider responses and noncanonical outbound schemas before network access. Exact clean Linux passes 321 full, 72 focused lifecycle, 198 provider, 60 adapter-focused, 30 security/evidence/traceability, and 228 pure tests. Independent workflow/safety approval was recorded at `2026-08-27T03:45:04Z`; independent test/release approval at `2026-08-27T03:45:05Z`. Live positive setup/test, invalid-key rejection without saving, valid-key reconfiguration, and post-browser-reload persistence passed; integration reload/unload/restart and scoped logs remain. Evidence: `docs/evidence/2026-08-24-loc-003-lm-studio-adapter.md`. |
| LOC-004 | Add provider setup/test UI | `REVIEW` | LOC-001, LOC-002 | Artifact `079d93f14fbbe9bd2da0658072d65a969f8b954d` adds admin-only, backend-originated provider list/test commands; no stored config, secret, entry ID, endpoint, entity state, or prompt reaches the panel. The test command accepts only a canonical configured connection ID, normalizes errors, and suppresses duplicate in-flight tests. The frontend strictly parses a versioned exact contract, exposes approved health language, includes the live-verified current-Core management route, sends no direct provider request, and covers loading/empty/error/test/accessibility states. Exact gates: 330 clean Linux, 16 focused WebSocket, 30 security/evidence/traceability, 228 pure, 61 browser; frontend build/sync identity, Ruff, canary, diff, and hashes passed. Independent workflow/safety approval was recorded at `2026-08-27T03:45:04Z`; independent test/release approval at `2026-08-27T03:45:05Z`. The approved UI is installed; positive setup/test, invalid-key rejection without saving, valid-key reconfiguration, and post-browser-reload persistence passed. Duplicate clicks, integration reload/unload/restart, Companion Android, screenshots, and logs remain. Evidence: `docs/evidence/2026-08-25-loc-004-provider-ui.md`. |
| LOC-005 | Add read-only entity/area/device catalog | `REVIEW` | LOC-001 | Artifact `238928b518ee9b9c3e64e5a4dd3d52b9fcae0288` adds an admin-only, read-only versioned registry catalog and Entities & Permissions panel surface. Synthetic gates: 332 full backend, 18 focused WebSocket, 69 frontend browser, Ruff, canary, build/sync/bundle identity passed. Live registry inventory, rename/removal/disable/unavailable tests, and independent reviews remain. Evidence: `docs/evidence/2026-08-27-loc-005-registry-catalog.md`; manifest: `docs/evidence/manifests/LOC-005/LOC-005-REGISTRY-CATALOG-001.json`. |
