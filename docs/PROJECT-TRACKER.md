...
Current resume point: **FND-013 remains in REVIEW after clean revision `982c58009471fb32cbdd97f5a1a821ce4b5ce579` passed test/release review but failed workflow/safety review. The working tree corrects all blocking findings: distinct local/off-device backup egress and restore ingress; transient authenticated credential entry versus backend-only stored secret/provider transport; separate same-version and actual-Core-upgrade IDs; prompt-injection control/corpus; storm/concurrency, supply-chain, and device-side-effect coverage. The corrected catalog defines seven data classes, thirteen nodes, eighteen flows, nineteen requirements, nineteen controls, and twenty-three tests; six integrity tests pass in isolation, including artifact-hash verification. Next: run the final full check set, commit the corrections, and obtain both independent re-reviews. FND-011 remains open only for an actual Core upgrade artifact, so FND-010 cannot close yet. Do not begin LOC-001 until FND-010 passes.**
...
| Field | Current value |
|---|---|
| Last completed | FND-007 live environment facts, including authenticated/firewall-scoped local-provider transport, same-subnet topology, automatic backups, emergency-kit custody, and restore policy |
| Active work | FND-013 redacted data-flow and requirement/control/test traceability |
...
| FND-013 | Define data-flow and control-to-test traceability records | Security + tracker | `REVIEW` | FND-007 | First clean revision passed test/release but failed workflow/safety review. Working tree corrects restore direction/zones, transient secret handling, compatibility scope, injection, storm/concurrency, supply-chain, and device-side-effect gaps. Corrected catalog: 7 data classes, 13 nodes, 18 flows, 19 requirements, 19 controls, 23 tests; six focused integrity tests pass including artifact hashes. Next: final full checks, corrected commit, both re-reviews, then manifest finalization |
