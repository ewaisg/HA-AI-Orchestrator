...
Current resume point: **Remote live records through `52c3a63` are preserved: LOC-003/LOC-004 positive setup, invalid-key rejection, valid-key reconfiguration, provider testing, and browser-reload persistence passed, and earlier LOC-005 artifact `238928b518ee9b9c3e64e5a4dd3d52b9fcae0288` passed its clean synthetic gates. A later 2026-08-28 review found noncanonical historical tool-call JSON and unqualified `Healthy` provider status. The rebased candidate fixes both and expands LOC-005 with stable registry identity, availability, search, and zero-AI-access UI. Current available gates pass 61 focused adapter, 229 pure, and 94 browser tests plus Ruff, canary, diff, build/sync, and bundle identity. Updated Home Assistant lifecycle tests require a restored Linux path because Docker Desktop's optional inference manager currently blocks engine startup. Next: publish this explicitly in-progress candidate, restore the Linux gate, obtain re-review, then install only the accepted bundle.**
...
| Active work | LOC-003/LOC-004 review remediations and expanded LOC-005 candidate; prior live acceptance evidence preserved |
...
| Next gate | Clean-Linux Home Assistant regression for the rebased candidate, then independent workflow/safety and test/release re-review; no updated install before acceptance |
...
| Repository | Local `main` is rebased on public `ewaisg/HA-AI-Orchestrator` `origin/main`; remote live records are preserved. The current candidate remains unaccepted pending Linux verification and re-review. The unrelated untracked `AI Orchestrator.iml` file is excluded from all commits and artifacts. |
...
| LOC-003 | Add authenticated LM Studio/OpenAI-compatible adapter | `IN PROGRESS` | LOC-001, LOC-002, ENV-003 | Artifact `53fe6781930f9264daf96ddd9f79f3aa7f691c33` had pre-live approvals and partial live success recorded on 2026-08-27. A later workflow/safety re-review proved historical assistant tool-call arguments could be stringified before canonical JSON validation, permitting noncanonical `Infinity`. The rebased candidate validates before serialization, uses `allow_nan=False`, maps rejection to `unsupported`, and proves zero network requests. Focused adapter tests pass 61; clean-Linux combined-candidate verification and re-review remain. Prior live evidence is retained but does not approve changed code. Evidence: `docs/evidence/2026-08-24-loc-003-lm-studio-adapter.md`. |
| LOC-004 | Add provider setup/test UI | `IN PROGRESS` | LOC-001, LOC-002 | Artifact `079d93f14fbbe9bd2da0658072d65a969f8b954d` had pre-live approvals and partial live success recorded on 2026-08-27. Later independent review found its list labeled every loaded connection healthy without a timestamped explicit test. The rebased candidate returns `not_tested` by default, stores normalized timestamped completed tests only in runtime, clears stale status during provider setup, and strictly validates/displays observation timestamps. Frontend passes 94 browser tests plus lint/type/build/sync identity; clean-Linux WebSocket regression and re-review remain. Prior live evidence is retained but does not approve changed code. Evidence: `docs/evidence/2026-08-25-loc-004-provider-ui.md`. |
| LOC-005 | Add read-only entity/area/device catalog | `IN PROGRESS` | LOC-001 | Earlier artifact `238928b518ee9b9c3e64e5a4dd3d52b9fcae0288` passed 332 clean backend, 18 focused WebSocket, and 69 browser tests. The rebased candidate expands the contract with stable entity-registry IDs, effective entity/device area source, availability classification, integration/domain metadata, search, responsive layout, explicit `AI access: none`, and stricter privacy omissions. Current browser suite passes 94 and pure suite 229; changed Home Assistant tests, independent re-review, and redacted live registry/rename/removal evidence remain. Evidence: `docs/evidence/2026-08-27-loc-005-registry-catalog.md` and `docs/evidence/2026-08-28-loc-005-read-only-catalog.md`. |
