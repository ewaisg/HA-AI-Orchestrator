...
Current resume point: **LOC-001 fourth candidate `2fbc55afd5d51e05ff861e1f900ee9781100bdea` was independently rejected at `2026-08-24T17:28:20Z` and `2026-08-24T17:31:33Z` because it read an attacker-controlled nested `.code` property before exact-type validation. The working tree now centrally requires exact `ProviderError` and exact `NormalizedError`, retrieves their stored payload without property dispatch, then accepts only an exact `ErrorCode`. New setup, initial setup, reauth, and reconfigure property-throwing tests pass within 66 focused clean-Linux tests. Create a new artifact, reproduce all gates, and repeat both reviews. No LM Studio transport, endpoint, credential, model, provider UI, or Home Assistant action is present.**
...
|---|---|---|---|---|
| LOC-001 | Complete local provider connection lifecycle on the validated skeleton | `REVIEW` | FND-010, LOC-002 | Fourth candidate passed 250 full/62 focused and its three hostile code classes, but both reviewers proved nested `.code` property access could still escape. The current tree uses a centralized non-dispatching exact outer/nested/code extractor and adds four property-throwing lifecycle regressions; 66 focused clean-Linux, 167 local provider/security/quality, 167 pure, Ruff, canary, and diff pass. Next: new artifact, full exact gates, fresh reviews. Evidence: `docs/evidence/2026-08-24-loc-001-provider-entry-lifecycle.md`. |
