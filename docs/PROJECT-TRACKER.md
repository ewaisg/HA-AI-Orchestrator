...
Current resume point: **FND-013 implementation is complete on the working tree and awaiting clean-commit independent workflow/safety plus test/release review. `docs/quality/DATA-FLOW-TRACEABILITY.md` and its JSON catalog/schema define redacted stable IDs for seven data classes, ten nodes, fourteen flows, nineteen requirements, fourteen controls, and eighteen tests. `tests/quality/test_traceability.py` enforces schema, ID/reference integrity, requirement coverage, evidence-backed verified statuses, and readable-map coverage. Working-tree checks: 85 pure tests passed; Ruff format/lint, canary, diff, and targeted privacy checks passed. The manifest remains incomplete until clean-commit reviews approve it. FND-011 remains open only for an actual Core upgrade artifact, so FND-010 cannot close yet. Do not begin LOC-001 until FND-010 passes.**
...
| FND-013 | Define data-flow and control-to-test traceability records | Security + tracker | `REVIEW` | FND-007 | Working-tree map, catalog/schema, four integrity tests, evidence record, and incomplete manifest added. 85 pure tests plus format/lint/canary/diff/privacy checks passed. Next: clean implementation commit, independent workflow/safety and test/release reviews, then finalize manifest and mark done |
