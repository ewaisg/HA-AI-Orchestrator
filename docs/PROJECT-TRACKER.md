...
Current resume point: **FND-007 now has resolved ENV-003 and ENV-009 plus partial ENV-010 live evidence. The owner-confirmed Home Assistant-to-LM Studio LAN path has missing/invalid-credential rejection, a restricted MCP-denying token, restored secret-backed positive HTTP `200` behavior, and an effective Private-only TCP `1234` firewall rule limited to one remote source with blocked edge traversal and disabled UDP. A post-firewall Home Assistant request returned HTTP `200`. Exact private identifiers are withheld. Home Assistant's Recommended automatic-backup policy is configured for daily encrypted backups retaining three copies; its first `87.89 MB` automatic backup completed to two locations, including Home Assistant Cloud. Next: confirm emergency-kit custody, adopt a safe restore-test policy, and resolve router/VLAN facts under ENV-010. Do not claim dependent FND-013 until FND-007 is complete. FND-011 remains open only for an actual Core upgrade artifact. Do not begin LOC-001 until FND-010 passes.**
...
| Evidence/input needed | Later FND-011 Core-upgrade evidence; emergency-kit custody confirmation, restore-test policy/evidence, and router/VLAN facts before FND-010 |
...
| FND-007 | Gather Phase 0 live environment facts | User + HA specialist | `IN PROGRESS` | — | ENV-001, ENV-003, ENV-004 discovery/action contract, Phase 0 ENV-007, and ENV-009 are resolved. `docs/evidence/2026-08-23-lm-studio-environment.md` records the authenticated, firewall-scoped provider path. `docs/evidence/2026-08-23-home-assistant-backup.md` records the Recommended daily encrypted backup policy and the first successful `87.89 MB` automatic backup to two locations, including Home Assistant Cloud. Exact private identifiers and recovery material were withheld. Next: confirm emergency-kit custody, adopt a safe restore-test policy, and resolve router/VLAN facts under partial ENV-010 |
