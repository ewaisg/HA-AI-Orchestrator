...
| Field | Current value |
|---|---|
| Last completed | LOC-008 installed and verified live on desktop. The owner's report that Chat and Providers "disappeared" was diagnosed as Home Assistant's Workbox service worker serving the previous bundle; the sections were never missing. After clearing the cached entry the panel served 95,427 bytes, all sections listed, the chat app shell was active (`chat-host`, `chat-mode`, transcript-only scrolling, page not scrolling), and a live prompt returned the real reply "New layout is live." That caching gap is now tracked as LOC-009. |
| Active work | LOC-008 owner confirmation on the Companion App for Android (keyboard behavior and spacing); LOC-009 cache-safe bundle updates; then LOC-007 Phase 1 release gate. |
...
| Next gate | Owner opens Chat in the Companion App for Android (clearing the app cache first, per LOC-009) and confirms only the message area scrolls, the input stays usable with the keyboard open, and spacing reads as consistent. Then implement LOC-009 cache-busting; reproduce bounded timeout/cancellation and duplicate-click protection; perform one disable/unavailable or area-change registry check; then run the LOC-007 quality/redaction/smoke gate. |
