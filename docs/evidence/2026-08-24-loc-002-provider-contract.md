...
- Assistant tool calls and tool-result messages preserve the exact provider-neutral call ID. A continuation rejects missing, duplicate, or unknown correlations, including multi-call histories. Historical assistant calls are revalidated against the uniquely named tools exposed by the request and their closed argument schemas.
...
The first remediation preserves and validates call IDs, adds the closed schema and fixed safe-error boundaries, expands the synthetic common cases, and returns `CTRL-PROVIDER-001` to `design_only` and `TEST-PROVIDER-CONTRACT` to `planned`. Candidate `4de9a453b353e63332251869b31d6f6c05dee13b` passed independent test/release review at `2026-08-24T07:29:46Z` but was rejected by workflow/safety at `2026-08-24T07:31:55Z`: continuation history did not reject duplicate exposed-tool names and did not revalidate historical assistant calls against the unique exposed tool and its argument schema. That rejection supersedes the test/release approval; no partial acceptance is inferred.
