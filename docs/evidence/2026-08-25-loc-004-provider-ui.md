...
The rebased candidate reports `not_tested` with no timestamp until an administrator completes an explicit backend test. Completed success and normalized failure results are stored only in integration runtime with an offset-aware completion timestamp; provider setup clears any stale observation. The list and test contracts expose the timestamp but no endpoint, model, token, config-entry ID, prompt, or state. The frontend requires an exact timestamped contract and never fabricates a completed observation after transport failure. The expanded frontend gate passes 94 browser tests plus lint, typecheck, build, sync, and byte identity. Earlier live results remain valid evidence for the earlier installed artifact, but they do not approve the changed candidate. Clean-Linux WebSocket regression and independent re-review remain required.
