...
## 2026-08-28 later review and superseding candidate

Both later independent reviewers rejected the published artifact despite its earlier approvals. The provider-list command labeled every loaded entry `healthy` without returning when that condition had been observed. Page reload could therefore display a stale or untested provider as healthy.

The superseding implementation reports `not_tested` with no timestamp until an administrator completes an explicit backend test. Completed success and normalized failure results are stored only in integration runtime with an offset-aware completion timestamp. The list and test contracts expose the timestamp but no endpoint, model, token, config-entry ID, prompt, or state. The frontend preserves the last evidenced provider status after Home Assistant communication failure and renders that failure separately.

## 2026-08-29 accepted pre-live candidate

Exact code candidate `989917fd4229f528c142a9ecebeeea3934c394da` additionally rejects malformed connection results and corrupted provider errors without overwriting prior status. Provider-test ownership survives unload/re-setup: a concurrent replacement remains blocked, a late old result cannot write status, the old request releases its own marker, and a current-provider test can then proceed. Exact booleans are enforced in the provider contract and again at the WebSocket boundary.
