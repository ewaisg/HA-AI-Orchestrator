...
export class WorkflowPreviewView extends LitElement {
  public static override properties = { hass: { attribute: false }, _workflow: { state: true }, _snapshot: { state: true }, _busy: { state: true }, _error: { state: true }, _result: { state: true } };
  ...
  public constructor() { super(); this._workflow = ""; this._snapshot = ""; this._busy = false; this._error = ""; }
  private readonly _reset = (): void => { this._invalidate(); this._workflow = ""; this._snapshot = ""; };
  private _invalidate(): void { this._sequence++; this._busy = false; this._error = ""; this._result = undefined; }
  ...
  protected override render(): TemplateResult {
    return html`<section aria-labelledby="preview-title">
      ...
      <div class="actions"><button class="primary" type="button" ?disabled=${this._busy || !this.hass || this.hass.connection?.connected === false || !this._workflow.trim() || !this._snapshot.trim()} @click=${this._preview}>${this._busy ? "Previewing…" : "Preview workflow"}</button><button type="button" @click=${this._reset}>Reset preview</button></div>
      <div role="status" aria-live="polite">${this._error ? html`<p class="error">${this._error}</p>` : nothing}${this._busy ? html`<p>Evaluating the supplied snapshot…</p>` : nothing}${this._result ? this._renderResult(this._result) : nothing}</div>
    </section>`;
  }
